...
// ─────────────────────────────────────────────────────────────────────────────
// SEED AWAL — hanya berjalan saat database benar-benar kosong (tabel users).
// ─────────────────────────────────────────────────────────────────────────────
...
async function ensureSeed(): Promise<void> {
  ...
  for (const [username, password, fullName, role] of seedUsers) {
  ...
  for (const [key, value] of seedSettings) {
    await SiteSettingModel.create({ id: await nextId('site_settings'), key, value });
  }

  for (const name of ['Lamaran', 'Ulang Tahun', 'Aqiqah', 'Grand Opening']) {
    await CategoryModel.create({ id: await nextId('categories'), name });
  }

  await setCounter('users', seedUsers.length);
  await setCounter('site_settings', seedSettings.length);
  await setCounter('categories', 4);
}
